...
public class PooledConnection implements PooledConnectionMBean {
  ...
  public boolean shouldForceReconnect(String username, String password) {
    ...
    noChangeInCredentials = (noChangeInCredentials || (username != null && username
      .equals(storedUsr)));
    
    noChangeInCredentials = noChangeInCredentials && ((password == null && storedPwd == null) || (
  ...
  protected void connectUsingDataSource() throws SQLException {
    ...
    if (poolProperties.getDataSource() instanceof javax.sql.XADataSource) {
      ...
      javax.sql.ConnectionPoolDataSource ds = (javax.sql.ConnectionPoolDataSource) poolProperties
        .getDataSource();
      if (usr != null && pwd != null) {
      ...
      throw new SQLException(
        "DataSource is of unknown class:" + (poolProperties.getDataSource() != null ?
                                             poolProperties.getDataSource().getClass() : "null"));
    }
  }
    ...
    try {
      if (driver == null) {
        if (log.isDebugEnabled()) {
          log.debug(
            "Instantiating driver using class: " + poolProperties.getDriverClassName() + " [url="
        ...
        if (poolProperties.getDriverClassName() == null) {
          ...
          driver = (java.sql.Driver) ClassLoaderUtil
            .loadClass(poolProperties.getDriverClassName(), PooledConnection.class.getClassLoader(),
                       Thread.currentThread().getContextClassLoader()).getConstructor()
            .newInstance();
        }
      }
  ...
  public void setDiscarded(boolean discarded) {
    if (this.discarded && !discarded) {
      throw new IllegalStateException(
        "Unable to change the state once the connection has been discarded");
    }
