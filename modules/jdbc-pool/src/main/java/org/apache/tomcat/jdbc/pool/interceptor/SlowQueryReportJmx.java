...
public class SlowQueryReportJmx extends SlowQueryReport
  ...
  protected static CompositeType getCompositeType() {
    if (SLOW_QUERY_TYPE == null) {
      try {
        SLOW_QUERY_TYPE = new CompositeType(SlowQueryReportJmx.class.getName(),
  ...
  protected void notifyJmx(String query, String type) {
    try {
      ...
      if (isNotifyPool()) {
        ...
        if (notifier != null) {
          Notification notification = new Notification(type, this, sequence,
  ...
  public ObjectName getObjectName(Class<?> clazz, String poolName)
    ...
    if (properties != null && properties.containsKey(objectNameAttribute)) {
      ...
      oname = new ObjectName(
        ConnectionPool.POOL_JMX_TYPE_PREFIX + clazz.getName() + ",name=" + poolName);
    }
    return oname;
  }
  ...
  protected void registerJmx() {
    try {
      ...
      if (isNotifyPool()) {
        ...
        log.warn(
          SlowQueryReport.class.getName() + "- No JMX support, composite type was not found.");
      }
    } catch (MalformedObjectNameException e) {
