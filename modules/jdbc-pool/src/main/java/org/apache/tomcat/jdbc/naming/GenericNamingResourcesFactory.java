...
public class GenericNamingResourcesFactory implements ObjectFactory {
  ...
  private static boolean setProperty(Object o, String name, String value) {
    ...
    try {
      ...
      for (int i = 0; i < methods.length; i++) {
        ...
        if (setter.equals(methods[i].getName()) && paramT.length == 1 && "java.lang.String"
          .equals(paramT[0].getName())) {
      ...
      for (int i = 0; i < methods.length; i++) {
        ...
        if (setter.equals(methods[i].getName()) && methods[i].getParameterTypes().length == 1) {
          ...
          if ("java.lang.Integer".equals(paramType.getName()) || "int"
            ...
            // Try a setFoo ( long )
          } else if ("java.lang.Long".equals(paramType.getName()) || "long"
            .equals(paramType.getName())) {
            try {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
          "IntrospectionUtils: SecurityException for " + o.getClass() + " " + name + "=" + value
            + ")", ex1);
      }
    } catch (IllegalAccessException iae) {
      if (log.isDebugEnabled()) {
        log.debug(
          "IntrospectionUtils: IllegalAccessException for " + o.getClass() + " " + name + "="
            + value + ")", iae);
      }
      ...
      if (log.isDebugEnabled()) {
        log.debug(
          "IntrospectionUtils: InvocationTargetException for " + o.getClass() + " " + name + "="
            + value + ")", ie);
      }
    }
    return false;
  ...
  public Object getObjectInstance(Object obj, Name name, Context nameCtx,
    ...
    Object o = ClassLoaderUtil.loadClass(type, GenericNamingResourcesFactory.class.getClassLoader(),
