...
public class DeltaManager extends ClusterManagerBase {
  ...
  protected void sendCreateSession(String sessionId, DeltaSession session) {
    if (cluster.getMembers().length > 0) {
      SessionMessage msg = new SessionMessageImpl(getName(), SessionMessage.EVT_SESSION_CREATED,
                                                  null, sessionId,
  ...
  protected void sendChangeSessionId(String newSessionID, String orgSessionID) {
    if (cluster.getMembers().length > 0) {
      try {
        ...
        SessionMessage msg = new SessionMessageImpl(getName(), SessionMessage.EVT_CHANGE_SESSION_ID,
                                                    data, orgSessionID, orgSessionID + "-" + System
          .currentTimeMillis());
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (findSession(session.getIdInternal()) == null) {
          ...
          if (log.isWarnEnabled()) {
            log
              .warn(sm.getString("deltaManager.loading.existing.session", session.getIdInternal()));
          }
        }
        add(session);
    ...
    try {
      if (cluster == null) {
        ...
        if (log.isInfoEnabled()) {
          ...
          log.info(sm.getString("deltaManager.registerCluster", getName(), type,
  ...
  public synchronized void getAllClusterSessions() {
    if (cluster != null && cluster.getMembers().length > 0) {
      ...
      SessionMessage msg = new SessionMessageImpl(this.getName(),
                                                  SessionMessage.EVT_GET_ALL_SESSIONS, null,
          ...
          for (SessionMessage smsg : receivedMessageQueue) {
            if (!stateTimestampDrop) {
              ...
              if (smsg.getEventType() != SessionMessage.EVT_GET_ALL_SESSIONS
                ...
                if (log.isWarnEnabled()) {
                  log.warn(
                    sm.getString("deltaManager.dropMessage", getName(), smsg.getEventTypeString(),
  ...
  public ClusterMessage requestCompleted(String sessionId, boolean expires) {
    ...
    try {
      ...
      if (session.isDirty()) {
        ...
        msg = new SessionMessageImpl(getName(), SessionMessage.EVT_SESSION_DELTA, session.getDiff(),
    ...
    if (msg == null) {
      if (!expires && !session.isPrimarySession()) {
        ...
        msg = new SessionMessageImpl(getName(), SessionMessage.EVT_SESSION_ACCESSED, null,
                                     sessionId, sessionId + "-" + System.currentTimeMillis());
        if (log.isDebugEnabled()) {
          log.debug(
    ...
    if (!expires && (msg == null)) {
      ...
      if (session.getMaxInactiveInterval() >= 0 && replDelta > (session.getMaxInactiveInterval()
        * 1000L)) {
        counterSend_EVT_SESSION_ACCESSED++;
        msg = new SessionMessageImpl(getName(), SessionMessage.EVT_SESSION_ACCESSED, null,
                                     sessionId, sessionId + "-" + System.currentTimeMillis());
  ...
  protected void sessionExpired(String id) {
    if (cluster.getMembers().length > 0) {
      ...
      SessionMessage msg = new SessionMessageImpl(getName(), SessionMessage.EVT_SESSION_EXPIRED,
  ...
  public void expireAllLocalSessions() {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Start expire all sessions " + getName() + " at " + timeNow + " sessioncount "
                  + sessions.length);
    }
    for (Session value : sessions) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("End expire sessions " + getName() + " expire processingTime " + (timeEnd - timeNow)
                  + " expired direct sessions: " + expireDirect + " expired direct sessions: "
                  + expireIndirect);
    }
  }
  ...
  protected void messageReceived(SessionMessage msg, Member sender) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
          sm.getString("deltaManager.receiveMessage.eventType", getName(), msg.getEventTypeString(),
                       sender));
      }
  ...
  protected void handleALL_SESSION_TRANSFERCOMPLETE(SessionMessage msg, Member sender) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        sm.getString("deltaManager.receiveMessage.transfercomplete", getName(), sender.getHost(),
  ...
  protected void handleGET_ALL_SESSIONS(SessionMessage msg, Member sender) throws IOException {
    ...
    SessionMessage newmsg = new SessionMessageImpl(name,
                                                   SessionMessage.EVT_ALL_SESSION_TRANSFERCOMPLETE,
                                                   null, "SESSION-STATE-TRANSFERRED",
  ...
  protected void handleALL_SESSION_NOCONTEXTMANAGER(SessionMessage msg, Member sender) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        sm.getString("deltaManager.receiveMessage.noContextManager", getName(), sender.getHost(),
                     Integer.valueOf(sender.getPort())));
    }
  ...
  protected void sendSessions(Member sender, Session[] currentSessions, long sendTimestamp)
    ...
    SessionMessage newmsg = new SessionMessageImpl(name, SessionMessage.EVT_ALL_SESSION_DATA, data,
