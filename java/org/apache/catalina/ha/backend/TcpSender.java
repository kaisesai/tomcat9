  ...
  public int send(String mess) throws Exception {
    ...
    for (int i = 0; i < connections.length; i++) {
      if (connections[i] == null) {
        try {
          ...
          connectionReaders[i] = new BufferedReader(
            new InputStreamReader(connections[i].getInputStream()));
          connectionWriters[i] = new BufferedWriter(
            new OutputStreamWriter(connections[i].getOutputStream()));
        } catch (Exception ex) {
      ...
      if (responseStatus == null) {
        ...
        responseStatus = responseStatus.substring(responseStatus.indexOf(' ') + 1, responseStatus
          .indexOf(' ', responseStatus.indexOf(' ') + 1));
