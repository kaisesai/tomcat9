...
public class ReplicationValve extends ValveBase implements ClusterValve {
  ...
  public void registerReplicationSession(DeltaSession session) {
    ...
    if (sessions != null) {
      if (!sessions.contains(session)) {
        if (log.isDebugEnabled()) {
          log.debug(
            sm.getString("ReplicationValve.crossContext.registerSession", session.getIdInternal(),
  ...
  public void invoke(Request request, Response response) throws IOException, ServletException {
    ...
    boolean isCrossContext = context != null && context instanceof StandardContext && context
      .getCrossContext();
    try {
      if (isCrossContext) {
        if (log.isDebugEnabled()) {
      ...
      if ((nrOfRequests % 100) == 0) {
        log.info(sm.getString("ReplicationValve.stats",
                              new Object[] {Long.valueOf(totalRequestTime / nrOfRequests),
                                ...
                                Long.valueOf(nrOfRequests), Long.valueOf(nrOfSendRequests),
  ...
  protected void createPrimaryIndicator(Request request) throws IOException {
    ...
    if ((id != null) && (id.length() > 0)) {
      ...
      if (session instanceof ClusterSession) {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
            sm.getString("ReplicationValve.session.indicator", request.getContext().getName(), id,
