...
public final class ApplicationFilterConfig implements FilterConfig, Serializable {
    ...
    if (this.filter != null) {
      try {
        ...
        context.getLogger().error(
          sm.getString("applicationFilterConfig.release", filterDef.getFilterName(),
      ...
      if (!context.getIgnoreAnnotations()) {
        try {
          ...
          ExceptionUtils.handleThrowable(t);
          context.getLogger().error(
            sm.getString("applicationFilterConfig.preDestroy", filterDef.getFilterName(),
                         filterDef.getFilterClass()), t);
        }
      }
    }
  ...
  private void registerJMX() {
    ...
    if (context instanceof StandardContext) {
      ...
      onameStr = domain + ":j2eeType=Filter,WebModule=" + webMod + ",name=" + filterName
        + ",J2EEApplication=" + standardContext.getJ2EEApplication() + ",J2EEServer="
    ...
    try {
      ...
      log.warn(
        sm.getString("applicationFilterConfig.jmxRegisterFail", getFilterClass(), getFilterName()),
        ex);
    }
  }
    ...
    if (oname != null) {
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(sm.getString("applicationFilterConfig.jmxUnregister", getFilterClass(),
                                 getFilterName()));
        }
      } catch (Exception ex) {
        log.warn(sm.getString("applicationFilterConfig.jmxUnregisterFail", getFilterClass(),
                              getFilterName()), ex);
      }
    }
  }
