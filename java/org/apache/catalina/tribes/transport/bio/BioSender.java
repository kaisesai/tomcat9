  ...
  public void sendMessage(byte[] data, boolean waitForAck) throws IOException {
    ...
    try {
      ...
      if (log.isTraceEnabled()) {
        log.trace(sm.getString("bioSender.send.again", getAddress().getHostAddress(),
    ...
    try {
      ...
      while ((i != -1) && (bytesRead < Constants.ACK_COMMAND.length)) {
        ...
        if (ackbuf.doesPackageExist()) {
          ...
          ackReceived = Arrays
            .equals(ackcmd, org.apache.catalina.tribes.transport.Constants.ACK_DATA);
          failAckReceived = Arrays
            .equals(ackcmd, org.apache.catalina.tribes.transport.Constants.FAIL_ACK_DATA);
      ...
      if (!ackReceived) {
        if (i == -1) {
          throw new IOException(sm.getString("bioSender.ack.eof", getAddress(),
                                             Integer.valueOf(socket.getLocalPort())));
        } else {
          throw new IOException(sm.getString("bioSender.ack.wrong", getAddress(),
                                             Integer.valueOf(socket.getLocalPort())));
        }
      ...
      String errmsg = sm
        .getString("bioSender.ack.missing", getAddress(), Integer.valueOf(socket.getLocalPort()),
