...
public class StaticMembershipProvider extends MembershipProviderBase
  ...
  protected static final StringManager sm = StringManager
    .getManager(StaticMembershipProvider.class);
  ...
  protected void startMembership(Member[] members) throws ChannelException {
    ...
    MemberMessage msg = new MemberMessage(membershipId, MemberMessage.MSG_START,
                                          service.getLocalMember(true));
  ...
  protected void ping() throws ChannelException {
    ...
    if (members.length > 0) {
      try {
        MemberMessage msg = new MemberMessage(membershipId, MemberMessage.MSG_PING,
                                              service.getLocalMember(true));
        Response[] resp = rpcChannel
          .send(members, msg, RpcChannel.ALL_REPLY, sendOptions, rpcTimeout);
        for (Response response : resp) {
          messageReceived(response.getMessage(), response.getSource());
        }
