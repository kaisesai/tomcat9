...
public class KubernetesMembershipProvider extends CloudMembershipProvider {
  ...
  public void start(int level) throws Exception {
    ...
    String clientCertificateFile = getEnv(CUSTOM_ENV_PREFIX + "CLIENT_CERT_FILE",
    ...
    if (clientCertificateFile == null) {
      ...
      try {
        ...
        streamProvider = new TokenStreamProvider(new String(bytes, StandardCharsets.US_ASCII),
                                                 caCertFile);
      } catch (IOException e) {
      ...
      streamProvider = new CertificateStreamProvider(clientCertificateFile, clientKeyFile,
    ...
    url = String
      .format("%s://%s:%s/api/%s/namespaces/%s/pods", protocol, masterHost, masterPort, ver,
              namespace);
