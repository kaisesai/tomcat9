...
public class McastServiceImpl extends MembershipProviderBase {
   ...
   */
  public McastServiceImpl(MemberImpl member, long sendFrequency, long expireTime, int port,
                          InetAddress bind, InetAddress mcastAddress, int ttl, int soTimeout,
  ...
  protected void setupSocket() throws IOException {
    ...
    socket.setLoopbackMode(
      localLoopbackDisabled); //hint if we want disable loop back(local machine) messages
    if (mcastBindAddress != null) {
        ...
        while (!success) {
          ...
          try {
            if (!success) {
              if (log.isInfoEnabled()) {
                log.info(
                  sm.getString("mcastServiceImpl.recovery.failed", Integer.toString(++attempt),
      ...
      while (doRunReceiver) {
        try {
          ...
          if (errorCounter == 0 && doRunReceiver) {
            ...
            log.debug("Error receiving mcast package" + (doRunReceiver ? ". Sleeping 500ms" : "."),
                      x);
          }
          if (doRunReceiver) {
            try {
