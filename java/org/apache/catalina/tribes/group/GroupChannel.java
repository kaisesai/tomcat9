...
public class GroupChannel extends ChannelInterceptorBase
    ...
    try {
      ...
      if (Logs.MESSAGES.isTraceEnabled()) {
        Logs.MESSAGES.trace("GroupChannel - Sent msg:" + new UniqueId(data.getUniqueId()) + " at "
                              + new java.sql.Timestamp(System.currentTimeMillis()) + " to " + Arrays
          .toNameString(destination));
        Logs.MESSAGES
          .trace("GroupChannel - Send Message:" + new UniqueId(data.getUniqueId()) + " is " + msg);
      }
  ...
  public void messageReceived(ChannelMessage msg) {
    ...
    try {
      if (Logs.MESSAGES.isTraceEnabled()) {
        Logs.MESSAGES.trace(
          "GroupChannel - Received msg:" + new UniqueId(msg.getUniqueId()) + " at "
            + new java.sql.Timestamp(System.currentTimeMillis()) + " from " + msg.getAddress()
            .getName());
      }
      ...
      if ((msg.getOptions() & SEND_OPTIONS_BYTE_MESSAGE) == SEND_OPTIONS_BYTE_MESSAGE) {
        ...
        try {
          fwd = XByteBuffer
            .deserialize(msg.getMessage().getBytesDirect(), 0, msg.getMessage().getLength());
        } catch (Exception sx) {
      ...
      if (Logs.MESSAGES.isTraceEnabled()) {
        Logs.MESSAGES
          .trace("GroupChannel delivered[" + delivered + "] id:" + new UniqueId(msg.getUniqueId()));
      }
  ...
  public synchronized void start(int svc) throws ChannelException {
    ...
    super.start(svc);
    monitorFuture = utilityExecutor.scheduleWithFixedDelay(new Runnable() {
      @Override
      public void run() {
        startHeartbeat();
      }
  ...
  protected void startHeartbeat() {
    if (heartbeat && (heartbeatFuture == null || (heartbeatFuture != null && heartbeatFuture
      .isDone()))) {
      ...
      heartbeatFuture = utilityExecutor
        .scheduleWithFixedDelay(new HeartbeatRunnable(), heartbeatSleeptime, heartbeatSleeptime,
  ...
  public void addChannelListener(ChannelListener channelListener) {
    if (!this.channelListeners.contains(channelListener)) {
      ...
      throw new IllegalArgumentException(
        sm.getString("groupChannel.listener.alreadyExist", channelListener,
