...
public class EncryptInterceptor extends ChannelInterceptorBase implements EncryptInterceptorMBean {
  ...
  private static final int[] DEC = {00, 01, 02, 03, 04, 05, 06, 07, 8, 9, -1, -1, -1, -1, -1, -1,
  ...
  private static BaseEncryptionManager createEncryptionManager(String algorithm,
    ...
    if ("GCM".equalsIgnoreCase(algorithmMode)) {
      return new GCMEncryptionManager(algorithm, new SecretKeySpec(encryptionKey, algorithmName),
                                      providerName);
    } else if ("CBC".equalsIgnoreCase(algorithmMode) || "OFB".equalsIgnoreCase(algorithmMode)
      || "CFB".equalsIgnoreCase(algorithmMode)) {
      return new BaseEncryptionManager(algorithm, new SecretKeySpec(encryptionKey, algorithmName),
                                       providerName);
    } else {
    ...
    while (null != interceptor) {
      if (interceptor instanceof TcpFailureDetector) {
        throw new ChannelConfigException(
          sm.getString("encryptInterceptor.tcpFailureDetector.ordering"));
      }
