      ...
      if (realFaultyMembers.length != 0) {
        ...
        if (backup.length == 0) {
          ...
          if (getLog().isWarnEnabled()) {
            getLog().warn(
              sm.getString("replicatedMap.unableReplicate.completely", key, Arrays.toString(backup),
    ...
    for (Entry<K, MapEntry<K, V>> e : innerMap.entrySet()) {
      ...
      if (entry.isPrimary()) {
        try {
          Member[] backup = getMapMembers();
          if (backup.length > 0) {
            MapMessage msg = new MapMessage(getMapContextName(), MapMessage.MSG_NOTIFY_MAPMEMBER,
      ...
      if (entry.getPrimary() == null && entry.isCopy() && entry.getBackupNodes() != null
        && entry.getBackupNodes().length > 0 && entry.getBackupNodes()[0]
