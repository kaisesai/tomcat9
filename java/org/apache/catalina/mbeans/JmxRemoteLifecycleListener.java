...
public class JmxRemoteLifecycleListener extends SSLHostConfig implements LifecycleListener {
  ...
  protected static final StringManager sm = StringManager
    .getManager(JmxRemoteLifecycleListener.class);
  ...
  protected void init() {
    ...
    String clientAuthValue = System
      .getProperty("com.sun.management.jmxremote.ssl.need.client.auth");
    ...
    if (Lifecycle.BEFORE_INIT_EVENT.equals(event.getType())) {
      ...
      if (rmiRegistrySSL) {
        ...
        if (rmiBindAddress == null) {
          ...
          registrySsf = new SslRmiServerBindSocketFactory(sslContext, getEnabledCiphers(),
                                                          getEnabledProtocols(),
                                                          getCertificateVerification()
                                                            == CertificateVerification.REQUIRED,
                                                          rmiBindAddress);
      ...
      if (rmiServerSSL) {
        ...
        if (rmiBindAddress == null) {
          ...
          serverSsf = new SslRmiServerBindSocketFactory(sslContext, getEnabledCiphers(),
                                                        getEnabledProtocols(),
                                                        getCertificateVerification()
                                                          == CertificateVerification.REQUIRED,
                                                        rmiBindAddress);
      ...
      csPlatform = createServer("Platform", rmiBindAddress, rmiRegistryPortPlatform,
                                rmiServerPortPlatform, env, registryCsf, registrySsf, serverCsf,
                                serverSsf);
  ...
  private JMXConnectorServer createServer(String serverName, String bindAddress,
                                          ...
                                          RMIClientSocketFactory registryCsf,
    ...
    try {
      RMIJRMPServerImpl server = new RMIJRMPServerImpl(rmiServerPortPlatform, serverCsf, serverSsf,
                                                       theEnv);
      ...
      try {
         ...
         * not to be used.
         */
        @SuppressWarnings("unused") JmxRegistry unused = new JmxRegistry(theRmiRegistryPort,
      ...
      log.info(
        sm.getString("jmxRemoteLifecycleListener.start", Integer.toString(theRmiRegistryPort),
  ...
  private static class JmxRegistry extends sun.rmi.registry.RegistryImpl {
    ...
    public JmxRegistry(int port, RMIClientSocketFactory csf, RMIServerSocketFactory ssf,
                       String jmxName, Remote jmxServer) throws RemoteException {
      super(port, csf, ssf);
