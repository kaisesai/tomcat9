...
public class CsrfPreventionFilter extends CsrfPreventionFilterBase {
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    filterConfig.getServletContext()
      .setAttribute(Constants.CSRF_NONCE_REQUEST_PARAM_NAME_KEY, nonceRequestParameterName);
  }
    ...
    if (request instanceof HttpServletRequest && response instanceof HttpServletResponse) {
      ...
      if (Constants.METHOD_GET.equals(req.getMethod()) && entryPoints
        ...
        if (log.isTraceEnabled()) {
          log.trace(
            "Skipping CSRF nonce-check for GET request to entry point " + getRequestedPath(req));
        }
      ...
      @SuppressWarnings("unchecked") LruCache<String> nonceCache = (session == null) ? null :
      ...
      if (!skipNonceCheck) {
        ...
        if (previousNonce == null) {
          if (log.isDebugEnabled()) {
            log.debug(
              "Rejecting request for " + getRequestedPath(req) + ", session " + (null == session ?
                                                                                 "(none)" :
                                                                                 session.getId())
      ...
      if (nonceCache == null) {
        if (log.isDebugEnabled()) {
          log.debug(
            "Creating new CSRF nonce cache with size=" + nonceCacheSize + " for session " + (
