...
public class Request implements HttpServletRequest {
    ...
    static {
        ...
        specialAttributes
          .put(Globals.PARAMETER_PARSE_FAILED_REASON_ATTR, new SpecialAttributeAdapter() {
              @Override
              public Object get(Request request, String name) {
        ...
        specialAttributes.put(Globals.SENDFILE_SUPPORTED_ATTR, new SpecialAttributeAdapter() {
            ...
            public Object get(Request request, String name) {
                return Boolean.valueOf(
                  request.getConnector().getProtocolHandler().isSendfileSupported() && request
                    .getCoyoteRequest().getSendfile());
            }
        ...
        if (context.getDispatchersUseEncodedPaths()) {
            if (pos >= 0) {
                relative = URLEncoder.DEFAULT
                  .encode(requestPath.substring(0, pos + 1), StandardCharsets.UTF_8) + path;
            } else {
        ...
        if (Globals.IS_SECURITY_ENABLED && name.equals(Globals.SENDFILE_FILENAME_ATTR)) {
            ...
            try {
                ...
                throw new SecurityException(
                  sm.getString("coyoteRequest.sendfileNotCanonical", value), e);
            }
    ...
    public AsyncContext startAsync(ServletRequest request, ServletResponse response) {
        if (!isAsyncSupported()) {
            ...
            log.warn(
              sm.getString("coyoteRequest.noAsync", StringUtils.join(getNonAsyncClassNames())),
              ise);
            throw ise;
        }
    ...
    private int nextSlash(char[] uri, int startPos) {
        ...
        while (pos < len) {
            if (uri[pos] == '/') {
              ...
              connector.getEncodedSolidusHandlingInternal() == EncodedSolidusHandling.DECODE
                && uri[pos] == '%' && pos + 2 < len && uri[pos + 1] == '2' && (uri[pos + 2] == 'f'
                || uri[pos + 2] == 'F')) {
                return pos;
            }
            pos++;
        ...
        if (userPrincipal instanceof TomcatPrincipal) {
            ...
            if (gssCredential != null) {
                ...
                try {
                    ...
                    log.warn(sm.getString("coyoteRequest.gssLifetimeFail", userPrincipal.getName()),
                             e);
                }
                if (left == 0) {
    ...
    private void parseParts(boolean explicit) {
        ...
        try {
            ...
            if (locationStr == null || locationStr.length() == 0) {
                location = ((File) context.getServletContext()
    ...
    protected Session doGetSession(boolean create) {
        ...
        if (requestedSessionId != null) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug(
                      sm.getString("request.session.failed", requestedSessionId, e.getMessage()),
                      e);
                } else {
                    log.info(
                      sm.getString("request.session.failed", requestedSessionId, e.getMessage()));
        ...
        boolean trackModesIncludesCookie = context.getServletContext()
        ...
        if (session != null && trackModesIncludesCookie) {
            Cookie cookie = ApplicationSessionCookieConfig
              .createSessionCookie(context, session.getIdInternal(), isSecure());
        ...
        try {
            ...
            if (len > 0) {
                ...
                try {
                    ...
                    if (context != null && context.getLogger().isDebugEnabled()) {
                        context.getLogger()
                          .debug(sm.getString("coyoteRequest.parseParameters"), ise);
                    }
                    return;
                } catch (IOException e) {
                    // Client disconnect
    ...
    protected byte[] readChunkedPostBody() throws IOException {
        ...
        while (len > -1) {
            ...
            if (connector.getMaxPostSize() >= 0 && (body.getLength() + len) > connector
              .getMaxPostSize()) {
                // Too much data
                checkSwallowInput();
