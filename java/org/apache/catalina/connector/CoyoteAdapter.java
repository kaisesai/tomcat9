...
public class CoyoteAdapter implements Adapter {
    ...
    protected static final boolean ALLOW_BACKSLASH = Boolean.parseBoolean(
      System.getProperty("org.apache.catalina.connector.CoyoteAdapter.ALLOW_BACKSLASH", "false"));
    ...
    private static final String POWERED_BY = "Servlet/4.0 JSP/2.3 " + "(" + ServerInfo
      .getServerInfo() + " Java/" + System.getProperty("java.vm.vendor") + "/" + System
    ...
    public static boolean normalize(MessageBytes uriMB) {
        ...
        if (((end - start) >= 2) && (b[end - 1] == (byte) '.')) {
            if ((b[end - 2] == (byte) '/') || ((b[end - 2] == (byte) '.') && (b[end - 3]
              == (byte) '/'))) {
                b[end] = (byte) '/';
                end++;
            }
    ...
    /**
     * 在解析了HTTP头之后，执行必要的处理，以使请求/响应对可以传递到容器管道的开头以进行处理。
     * <p>
    ...
    protected boolean postParseRequest(org.apache.coyote.Request req, Request request,
      ...
      throws IOException, ServletException {
        ...
        if (!redirectPathMB.isNull()) {
            String redirectPath = URLEncoder.DEFAULT
              .encode(redirectPathMB.toString(), StandardCharsets.UTF_8);
            String query = request.getQueryString();
            if (request.isRequestedSessionIdFromURL()) {
                // This is not optimal, but as this is not very common, it
                // shouldn't matter
                redirectPath = redirectPath + ";" + SessionConfig
                  .getSessionUriParamName(request.getContext()) + "=" + request
        ...
        if (username != null) {
            ...
            if (req.getRemoteUserNeedsAuthorization()) {
                ...
                if (!(authenticator instanceof AuthenticatorBase)) {
                    ...
                    request
                      .setUserPrincipal(request.getContext().getRealm().authenticate(username));
                }
    ...
    protected void parsePathParameters(org.apache.coyote.Request req, Request request) {
        ...
        while (semicolon > -1) {
            ...
            int pathParamEnd = ByteChunk
              .findBytes(uriBC.getBuffer(), start + pathParamStart, end, new byte[] {';', '/'});
    ...
    protected void parseSessionSslId(Request request) {
        if (request.getRequestedSessionId() == null && SSL_ONLY
          .equals(request.getServletContext().getEffectiveSessionTrackingModes())
