...
public class CachedResource implements WebResource {
  ...
  protected boolean validateResource(boolean useClassLoaderResources) {
    ...
    if (!root.isPackedWarFile()) {
      WebResource webResourceInternal = root
        .getResourceInternal(webAppPath, useClassLoaderResources);
  ...
  public URL getURL() {
    ...
    try {
      CachedResourceURLStreamHandler handler = new CachedResourceURLStreamHandler(resourceURL, root,
                                                                                  webAppPath,
