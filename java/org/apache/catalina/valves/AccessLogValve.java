...
public class AccessLogValve extends AbstractAccessLogValve {
  ...
  public synchronized void backgroundProcess() {
    ...
    if (rotatable && checkForOldLogs && maxDays > 0) {
      long deleteIfLastModifiedBefore = System.currentTimeMillis() - (maxDays * 24L * 60 * 60
        * 1000);
  ...
  protected synchronized void open() {
    ...
    try {
      writer = new PrintWriter(
        new BufferedWriter(new OutputStreamWriter(new FileOutputStream(pathname, true), charset),
                           128000), false);
