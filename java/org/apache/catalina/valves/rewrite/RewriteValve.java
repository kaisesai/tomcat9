        ...
        if (map == null) {
          try {
            ...
            throw new IllegalArgumentException(
              sm.getString("rewriteValve.invalidMapClassName", line));
          }
        }
        if (tokenizer.hasMoreTokens()) {
    ...
    while (true) {
      try {
        ...
        if (result instanceof RewriteRule) {
          ...
          if (containerLog.isDebugEnabled()) {
            containerLog.debug(
              "Add rule with pattern " + rule.getPatternString() + " and substitution " + rule
          ...
          for (RewriteCond condition : conditions) {
            if (containerLog.isDebugEnabled()) {
              ...
              containerLog.debug(
                "Add condition " + cond.getCondPattern() + " test " + cond.getTestString()
                  + " to rule with pattern " + rule.getPatternString() + " and substitution " + rule
                  .getSubstitutionString() + (cond.isOrnext() ? " [OR]" : "") + (cond.isNocase() ?
                                                                                 " [NC]" : ""));
            }
    ...
    try {
      ...
      for (int i = 0; i < rules.length; i++) {
        ...
        if (newtest != null && !test.equals(newtest.toString())) {
          if (containerLog.isDebugEnabled()) {
            containerLog.debug("Rewrote " + test + " as " + newtest + " with rule pattern " + rule
              .getPatternString());
          }
          if (rule.isHost()) {
            host = newtest;
        ...
        if (rule.isRedirect() && newtest != null) {
          ...
          StringBuilder urlStringEncoded = new StringBuilder(
          ...
          if (context && urlStringEncoded.charAt(0) == '/' && !UriUtil
            .hasScheme(urlStringEncoded)) {
        ...
        if (!done) {
          ...
          try {
            if (!connector.getProtocolHandler().getAdapter()
              .prepare(request.getCoyoteRequest(), response.getCoyoteResponse())) {
              return;
            }
          } catch (Exception e) {
