...
public class JNDIRealm extends RealmBase {
  ...
  private String[] getCipherSuitesArray() {
    ...
    if (this.cipherSuites.trim().isEmpty()) {
      ...
      containerLog
        .debug(sm.getString("jndiRealm.cipherSuites", Arrays.toString(this.cipherSuitesArray)));
    }
   ...
   */
  protected User getUserByPattern(JNDIConnection connection, String username, String credentials,
                                  String[] attrIds, int curUserPattern) throws NamingException {
  ...
  protected User getUserBySearch(JNDIConnection connection, String username, String[] attrIds)
    ...
    NamingEnumeration<SearchResult> results = connection.context
      .search(userBase, filter, constraints);
  ...
  protected List<String> getRoles(JNDIConnection connection, User user) throws NamingException {
    ...
    String filter = connection.roleFormat
      .format(new String[] {doRFC2254Encoding(dn), username, userRoleId});
    ...
    NamingEnumeration<SearchResult> results = searchAsUser(connection.context, user, base, filter,
    ...
    if (getRoleNested()) {
      ...
      while (!newGroups.isEmpty()) {
        Map<String, String> newThisRound
          = new HashMap<>(); // Stores the groups we find in this iteration
        
        for (Entry<String, String> group : newGroups.entrySet()) {
          filter = connection.roleFormat.format(
            new String[] {doRFC2254Encoding(group.getKey()), group.getValue(), group.getValue()});
          
          if (containerLog.isTraceEnabled()) {
            containerLog.trace(
              "Perform a nested group search with base " + roleBase + " and filter " + filter);
          }
   ...
   * @throws NamingException if a directory server error occurs
   */
  private NamingEnumeration<SearchResult> searchAsUser(DirContext context, User user, String base,
                                                       String filter, SearchControls controls,
   ...
   * @throws NamingException if a directory server error occurs
   */
  private ArrayList<String> addAttributeValues(String attrId, Attributes attrs,
                                               ArrayList<String> values) throws NamingException {
   ...
   * @throws NamingException if a directory server error occurs
   */
  protected Principal getPrincipal(JNDIConnection connection, String username,
    ...
    try {
      ...
      if (gssCredential != null && isUseDelegatedCredential()) {
        ...
        restoreEnvironmentParameter(context, "javax.security.sasl.server.authentication",
    ...
    if (user != null) {
      return new GenericPrincipal(user.getUserName(), user.getPassword(), roles, null, null,
                                  gssCredential);
    }
  ...
  private SSLSocketFactory createSSLSocketFactoryFromClassName(String className) {
    try {
      ...
      if (o instanceof SSLSocketFactory) {
        ...
        throw new IllegalArgumentException(
          sm.getString("jndiRealm.invalidSslSocketFactory", className));
      }
    } catch (ReflectiveOperationException | SecurityException e) {
      throw new IllegalArgumentException(
        sm.getString("jndiRealm.invalidSslSocketFactory", className), e);
    }
  }
  ...
  private SSLSocketFactory createSSLContextFactoryFromProtocol(String protocol) {
    try {
      ...
      throw new IllegalArgumentException(
        sm.getString("jndiRealm.invalidSslProtocol", protocol, allowedProtocols), e);
    }
  }
  ...
  protected String[] parseUserPatternString(String userPatternString) {
    ...
    if (userPatternString != null) {
      ...
      while (startParenLoc > -1) {
        ...
        while ((userPatternString.charAt(startParenLoc + 1) == '|') || (startParenLoc != 0
      ...
      try {
        ...
        if (pathComponent.length() < 1) {
          throw new InvalidNameException(
            "Search returned unparseable absolute name: " + resultName);
        }
