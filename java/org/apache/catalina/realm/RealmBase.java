...
public abstract class RealmBase extends LifecycleMBeanBase implements Realm {
  ...
  private static X509UsernameRetriever createUsernameRetriever(String className)
    ...
    try {
      @SuppressWarnings("unchecked") Class<? extends X509UsernameRetriever> clazz
  ...
  public Principal authenticate(String username, String clientDigest, String nonce, String nc,
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        "Digest : " + clientDigest + " Username:" + username + " ClientDigest:" + clientDigest
          + " nonce:" + nonce + " nc:" + nc + " cnonce:" + cnonce + " qop:" + qop + " realm:"
          + realm + "md5a2:" + md5a2 + " Server digest:" + serverDigest);
    }
  ...
  public SecurityConstraint[] findSecurityConstraints(Request request, Context context) {
    ...
    for (i = 0; i < constraints.length; i++) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
          "  Checking constraint '" + constraints[i] + "' against " + method + " " + uri + " --> "
          ...
          if (pattern.startsWith("/") && pattern.endsWith("/*") && pattern.length() >= longest) {
            ...
            if (pattern.length() == 2) {
              ...
              pattern.length() - 2 == uri.length() && pattern
                .regionMatches(0, uri, 0, pattern.length() - 2))) {
              matched = true;
              length = pattern.length();
            }
          }
  ...
  public boolean hasResourcePermission(Request request, Response response,
    ...
    for (SecurityConstraint constraint : constraints) {
      ...
      if (constraint.getAuthenticatedUsers() && principal != null) {
        ...
        status = true;
      } else if (roles.length == 0 && !constraint.getAllRoles() && !constraint
