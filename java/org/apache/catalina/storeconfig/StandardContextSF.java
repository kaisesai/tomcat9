...
public class StandardContextSF extends StoreFactoryBase {
   ...
   */
  protected void storeContextSeparate(PrintWriter aWriter, int indent, StandardContext aContext)
    throws Exception {
    ...
    if (configFile != null) {
      ...
      try (
        ...
        PrintWriter writer = new PrintWriter(
          new OutputStreamWriter(fos, getRegistry().getEncoding()))
      ) {
        storeXMLHead(writer);
    ...
    if (mover != null) {
      ...
      if (dir != null && dir.isDirectory() && (!dir.canWrite())) {
        throw new IOException(
          sm.getString("standardContextSF.cannotWriteFile", mover.getConfigSave()));
      }
      if (log.isInfoEnabled()) {
   ...
   */
  protected String[] filterWatchedResources(StandardContext context, String[] wresources)
    throws Exception {
    File configBase = configBase(context);
    String confContext = new File(System.getProperty("catalina.base"), "conf/context.xml")
      .getCanonicalPath();
    String confWeb = new File(System.getProperty("catalina.base"), "conf/web.xml")
      .getCanonicalPath();
