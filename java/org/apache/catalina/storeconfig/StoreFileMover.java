    ...
    if (!configNew.getParentFile().exists()) {
      if (!configNew.getParentFile().mkdirs()) {
        throw new IllegalStateException(
          sm.getString("storeFileMover.directoryCreationError", configNew));
      }
    }
    ...
    if (configOld.renameTo(configSave)) {
      if (!configNew.renameTo(configOld)) {
        ...
        throw new IOException(
          sm.getString("storeFileMover.renameError", configNew.getAbsolutePath(),
      ...
      if (!configOld.exists()) {
        ...
        throw new IOException(
          sm.getString("storeFileMover.renameError", configOld.getAbsolutePath(),
