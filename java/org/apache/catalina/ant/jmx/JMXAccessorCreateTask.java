...
public class JMXAccessorCreateTask extends JMXAccessorTask {
  ...
  protected void jmxCreate(MBeanServerConnection jmxServerConnection, String name)
    ...
    if (classLoader != null && !classLoader.isEmpty()) {
      if (isEcho()) {
        handleOutput(
          "create MBean " + name + " from class " + className + " with classLoader " + classLoader);
      }
      if (args == null) {
        jmxServerConnection
          .createMBean(className, new ObjectName(name), new ObjectName(classLoader));
      } else {
        jmxServerConnection
          .createMBean(className, new ObjectName(name), new ObjectName(classLoader), argsA, sigA);
      }
