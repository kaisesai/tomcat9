...
public final class HTMLManagerServlet extends ManagerServlet {
  ...
  protected void list(HttpServletRequest request, HttpServletResponse response, String message,
    ...
    args[3] = // External link
      (request.getContextPath() + "/" + smClient
    ...
    args[5] = // External link
      (request.getContextPath() + "/" + smClient.getString("htmlManagerServlet.helpManagerFile"));
      ...
      if (ctxt != null) {
        ...
        args[0] = // External link
          "<a href=\"" + URLEncoder.DEFAULT.encode(contextPath + "/", StandardCharsets.UTF_8)
            + "\" " + Constants.REL_EXTERNAL + ">" + Escape.htmlElementContent(displayPath)
            + "</a>";
        ...
        args[4] = Escape.htmlElementContent(
          response.encodeURL(request.getContextPath() + "/html/sessions?" + pathVersion));
        ...
        args[1] = appsStart;
        args[2] = Escape.htmlElementContent(
          response.encodeURL(request.getContextPath() + "/html/stop?" + pathVersion));
        args[3] = appsStop;
        args[4] = Escape.htmlElementContent(
  ...
  protected void sslConnectorTrustedCerts(HttpServletRequest request, HttpServletResponse response,
    ...
    getServletContext().getRequestDispatcher(connectorTrustedCertsJspPath)
   ...
   * @throws IOException      An IO error occurred
   */
  protected void doSessions(ContextName cn, HttpServletRequest req, HttpServletResponse resp,
                            StringManager smClient) throws ServletException, IOException {
  ...
  protected List<Session> getSessionsForName(ContextName cn, StringManager smClient) {
    ...
    if (null == ctxt) {
      throw new IllegalArgumentException(smClient.getString("managerServlet.noContext", Escape
        .htmlElementContent(cn.getDisplayName())));
    }
    Manager manager = ctxt.getManager();
   ...
   * @throws IOException      An IO error occurred
   */
  protected void displaySessionsListPage(ContextName cn, HttpServletRequest req,
    ...
    if (null != sortBy && !"".equals(sortBy.trim())) {
      ...
      if (comparator != null) {
        ...
        try {
          ...
          req
            .setAttribute(APPLICATION_ERROR, "Can't sort session list: one session is invalidated");
        }
      } else {
  ...
  protected void displaySessionDetailPage(HttpServletRequest req, HttpServletResponse resp,
                                          ContextName cn, String sessionId, StringManager smClient)
    throws ServletException, IOException {
    ...
    getServletContext().getRequestDispatcher(resp.encodeURL(sessionDetailJspPath))
      .include(req, resp);
  }
  ...
  protected boolean removeSessionAttribute(ContextName cn, String sessionId, String attributeName,
    ...
    try {
      ...
      if (debug >= 1) {
        log("Cannot remote attribute '" + attributeName + "' for invalidated session id "
              + sessionId);
      }
    }
    return wasPresent;
