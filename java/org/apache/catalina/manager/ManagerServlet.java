...
public class ManagerServlet extends HttpServlet implements ContainerServlet {
  ...
  protected static boolean validateContextName(ContextName cn, PrintWriter writer,
  ...
  protected synchronized void save(PrintWriter writer, String path, StringManager smClient) {
    ...
    if ((path == null) || path.length() == 0 || !path.startsWith("/")) {
      ...
      try {
        Boolean result = (Boolean) mBeanServer
          .invoke(storeConfigOname, "store", new Object[] {context},
   ...
   */
  protected synchronized void deploy(PrintWriter writer, String config, ContextName cn, String tag,
    ...
    if (debug >= 1) {
      if (config == null) {
        ...
        log("deploy: Deploying web application '" + cn + "' " + "with context configuration at '"
              + config + "'");
      }
    }
        ...
        try {
          ...
          if (update && tag == null) {
            ...
            if (!uploadedWar.renameTo(deployedWar)) {
              writer
                .println(smClient.getString("managerServlet.renameFail", uploadedWar, deployedWar));
              return;
            }
          }
          if (tag != null) {
   ...
   * @param smClient i18n messages using the locale of the client
   */
  protected void deploy(PrintWriter writer, String config, ContextName cn, String war,
                        boolean update, StringManager smClient) {
  ...
  private void writeDeployResult(PrintWriter writer, StringManager smClient, String name,
                                 String displayPath) {
    ...
    for (Container container : contexts) {
      ...
      if (context != null) {
        ...
        if (context.getState().isAvailable()) {
          parts = Arrays
            .asList(displayPath, "running", "" + context.getManager().findSessions().length,
                    context.getDocBase());
        } else {
   ...
   */
  protected void printResources(PrintWriter writer, String prefix,
                                javax.naming.Context namingContext, String type,
                                StringManager smClient) {
    try {
      ...
      while (items.hasMore()) {
        ...
        if (obj instanceof javax.naming.Context) {
          printResources(writer, prefix + item.getName() + "/", (javax.naming.Context) obj, type,
                         smClient);
        } else {
          if (type != null && (obj == null || !IntrospectionUtils
            .isInstance(obj.getClass(), type))) {
            continue;
          }
  ...
  protected void sessions(PrintWriter writer, ContextName cn, int idle, StringManager smClient) {
    ...
    try {
      ...
      if (context == null) {
        writer.println(
          smClient.getString("managerServlet.noContext", Escape.htmlElementContent(displayPath)));
        return;
      }
      Manager manager = context.getManager();
      if (manager == null) {
        writer.println(
          smClient.getString("managerServlet.noManager", Escape.htmlElementContent(displayPath)));
        return;
      }
      int maxCount = 60;
      int histoInterval = 1;
      ...
      writer
        .println(smClient.getString("managerServlet.sessiondefaultmax", "" + maxInactiveInterval));
      ...
      if (timeout[0] > 0) {
        writer.println(smClient.getString("managerServlet.sessiontimeout", "<" + histoInterval,
                                          "" + timeout[0]));
      }
      for (int i = 1; i < maxCount; i++) {
        if (timeout[i] > 0) {
          writer.println(smClient.getString("managerServlet.sessiontimeout",
                                            "" + (i) * histoInterval + " - <"
                                              + (i + 1) * histoInterval, "" + timeout[i]));
        }
      }
      if (timeout[maxCount] > 0) {
        writer.println(smClient.getString("managerServlet.sessiontimeout",
                                          ">=" + maxCount * histoInterval, "" + timeout[maxCount]));
      }
      if (notimeout > 0) {
        writer
          .println(smClient.getString("managerServlet.sessiontimeout.unlimited", "" + notimeout));
      }
      if (idle >= 0) {
        writer.println(
          smClient.getString("managerServlet.sessiontimeout.expired", ">" + idle, "" + expired));
      }
    } catch (Throwable t) {
      ExceptionUtils.handleThrowable(t);
   ...
   * @param smClient i18n support for current client's locale
   */
  protected void expireSessions(PrintWriter writer, ContextName cn, HttpServletRequest req,
                                StringManager smClient) {
    int idle = -1;
   ...
   */
  protected void uploadWar(PrintWriter writer, HttpServletRequest request, File war,
      ...
      if (Boolean.TRUE.equals(connector.getProperty("SSLEnabled"))) {
        ...
        for (SSLHostConfig sslHostConfig : sslHostConfigs) {
          ...
          result.put(name, new ArrayList<>(
            new LinkedHashSet<>(Arrays.asList(sslHostConfig.getEnabledCiphers()))));
        }
      } else {
    ...
    for (Connector connector : connectors) {
      if (Boolean.TRUE.equals(connector.getProperty("SSLEnabled"))) {
        ...
        for (SSLHostConfig sslHostConfig : sslHostConfigs) {
          ...
          if (sslHostConfig.getOpenSslContext().longValue() == 0) {
            // Not set. Must be JSSE based.
            SSLContext sslContext = sslHostConfig.getCertificates().iterator().next()
              .getSslContext();
