...
public class NamingResourcesImpl extends LifecycleMBeanBase
  ...
  public void addEnvironment(ContextEnvironment environment) {
    ...
    if (injectionTargets != null && injectionTargets.size() > 0 && (value == null
      || value.length() == 0)) {
      return;
    }
    ...
    if (!checkResourceType(environment)) {
      throw new IllegalArgumentException(
        sm.getString("namingResources.resourceTypeFail", environment.getName(),
                     environment.getType()));
    }
  ...
  private Class<?> getSetterType(Class<?> clazz, String name) {
    ...
    if (methods != null && methods.length > 0) {
      for (Method method : methods) {
        if (Introspection.isValidSetter(method) && Introspection.getPropertyName(method)
          .equals(name)) {
