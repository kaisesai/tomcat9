...
public class FormAuthenticator extends AuthenticatorBase {
  ...
  protected boolean doAuthenticate(Request request, HttpServletResponse response)
    ...
    if (requestURI == null) {
      if (landingPage == null) {
        response
          .sendError(HttpServletResponse.SC_BAD_REQUEST, sm.getString("authenticator.formlogin"));
      } else {
        // Make the authenticator think the user originally requested
        // the landing page
        String uri = request.getContextPath() + landingPage;
  ...
  protected void forwardToLoginPage(Request request, HttpServletResponse response,
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        sm.getString("formAuthenticator.forwardLogin", request.getRequestURI(), request.getMethod(),
  ...
  protected boolean restoreRequest(Request request, Session session) throws IOException {
    ...
    while (names.hasNext()) {
      ...
      // BZ 43687
      if (!("If-Modified-Since".equalsIgnoreCase(name) || (cacheable && "If-None-Match"
