...
class TagFileProcessor {
  ...
  public static TagInfo parseTagFileDirectives(ParserController pc, String name, String path,
                                               Jar jar, TagLibraryInfo tagLibInfo)
    ...
    TagFileDirectiveVisitor tagFileVisitor = new TagFileDirectiveVisitor(pc.getCompiler(),
                                                                         tagLibInfo, name, path);
   ...
   */
  private Class<?> loadTagFile(Compiler compiler, String tagFilePath, TagInfo tagInfo,
                               PageInfo parentPageInfo) throws JasperException {
      ...
      if (tagFilePath.startsWith("/META-INF/")) {
        try {
          tagJar = compiler.getCompilationContext()
            .getTldResourcePath(tagInfo.getTagLibrary().getURI()).openJar();
        ...
        try {
          ...
          if (wrapper == null) {
            wrapper = new JspServletWrapper(ctxt.getServletContext(), ctxt.getOptions(),
                                            tagFilePath, tagInfo, ctxt.getRuntimeContext(), tagJar);
          ...
          try {
            ...
            if (tagIns instanceof JspSourceDependent) {
              for (Entry<String, Long> entry : ((JspSourceDependent) tagIns).getDependants()
                .entrySet()) {
  ...
  private static class TagFileDirectiveVisitor extends Node.Visitor {
    ...
    private static final JspUtil.ValidAttribute[] tagDirectiveAttrs = {
      new JspUtil.ValidAttribute("display-name"), new JspUtil.ValidAttribute("body-content"),
      new JspUtil.ValidAttribute("dynamic-attributes"), new JspUtil.ValidAttribute("small-icon"),
      new JspUtil.ValidAttribute("large-icon"), new JspUtil.ValidAttribute("description"),
      new JspUtil.ValidAttribute("example"), new JspUtil.ValidAttribute("pageEncoding"),
      new JspUtil.ValidAttribute("language"), new JspUtil.ValidAttribute("import"),
    ...
    public TagFileDirectiveVisitor(Compiler compiler, TagLibraryInfo tagLibInfo, String name,
                                   String path) {
    ...
    public void visit(Node.TagDirective n) throws JasperException {
      ...
      if (bodycontent != null && !bodycontent.equalsIgnoreCase(TagInfo.BODY_CONTENT_EMPTY)
        && !bodycontent.equalsIgnoreCase(TagInfo.BODY_CONTENT_TAG_DEPENDENT) && !bodycontent
    ...
    public void visit(Node.AttributeDirective n) throws JasperException {
      ...
      if (("2.0".equals(tagLibInfo.getRequiredVersion()) || ("1.2"
        .equals(tagLibInfo.getRequiredVersion()))) && (deferredMethodSpecified || deferredMethod
        || deferredValueSpecified || deferredValue)) {
        err.jspError("jsp.error.invalid.version", path);
      }
      
      TagAttributeInfo tagAttributeInfo = new TagAttributeInfo(attrName, required, type,
                                                               rtexprvalue, fragment, null,
                                                               deferredValue, deferredMethod,
                                                               deferredValueType,
    ...
    public void visit(Node.VariableDirective n) throws JasperException {
      ...
      variableVector
        .addElement(new TagVariableInfo(nameGiven, nameFromAttribute, className, declare, scope));
    }
    ...
    public TagInfo getTagInfo() throws JasperException {
      ...
      return new JasperTagInfo(name, tagClassName, bodycontent, description, tagLibInfo, null,
                               tagAttributeInfo, displayName, smallIcon, largeIcon,
    ...
    private void checkUniqueName(String name, String type, Node n, TagAttributeInfo attr)
      throws JasperException {
    ...
    void postCheck() throws JasperException {
      ...
      for (Entry<String, NameEntry> entry : nameFromTable.entrySet()) {
        ...
        if (nameEntry == null) {
          ...
          if (!"java.lang.String".equals(tagAttr.getTypeName()) || !tagAttr.isRequired() || tagAttr
            .canBeRequestTime()) {
      ...
      if (tagFileInfo != null) {
        ...
        if (tagFilePath.startsWith("/META-INF/")) {
          ...
          try (Jar jar = tldResourcePath.openJar()) {
            ...
            if (jar != null) {
              // Add TLD
              pageInfo.addDependant(jar.getURL(tldResourcePath.getEntryName()), Long
                .valueOf(jar.getLastModified(tldResourcePath.getEntryName())));
              // Add Tag
