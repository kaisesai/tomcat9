...
public abstract class Compiler {
  ...
  protected Map<String, SmapStratum> generateJava() throws Exception {
    ...
    pageInfo = new PageInfo(new BeanRepository(ctxt.getClassLoader(), errDispatcher),
                            ctxt.getJspFile(), ctxt.isTagFile());
    ...
    if (ctxt.isTagFile()) {
      try {
        double libraryVersion = Double
          .parseDouble(ctxt.getTagInfo().getTagLibrary().getRequiredVersion());
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        ...
        log.debug("Generated " + javaFileName + " total=" + (t4 - t1) + " generate=" + (t4 - t3)
                    + " validate=" + (t2 - t1));
      }
