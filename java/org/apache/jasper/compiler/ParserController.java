...
class ParserController implements TagConstants {
  ...
  private Node.Nodes doParse(String inFileName, Node parent, Jar jar)
    ...
    if (parent != null) {
      ...
      if (jar == null) {
        ...
        compiler.getPageInfo()
          .addDependant(jar.getURL(entry), Long.valueOf(jar.getLastModified(entry)));
    ...
    if (isXml) {
      ...
      parsedPage = JspDocumentParser
        .parse(this, absFileName, jar, parent, isTagFile, directiveOnly, sourceEnc,
               jspConfigPageEnc, isEncodingSpecifiedInProlog, isBomPresent);
    } else {
      // Standard syntax
      try (
        InputStreamReader inStreamReader = JspUtil
          .getReader(absFileName, sourceEnc, jar, ctxt, err, skip)
      ) {
   ...
   */
  private String getPageEncodingForJspSyntax(JspReader jspReader, Mark startMark)
