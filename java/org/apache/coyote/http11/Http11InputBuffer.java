...
public class Http11InputBuffer implements InputBuffer, ApplicationBufferHandler {
    ...
    /**
     * 阅读请求行。 该功能是在HTTP请求标头解析期间使用。 不要尝试使用它读取请求正文。
     * <p>
    ...
    private String parseInvalid(int startPos, ByteBuffer buffer) {
        ...
        String result = HeaderUtil
          .toPrintableString(buffer.array(), buffer.arrayOffset() + startPos,
   ...
   */
    private boolean fill(boolean block) throws IOException {
        ...
        if (log.isDebugEnabled()) {
            log.debug(
              "Before fill(): [" + parsingHeader + "], parsingRequestLine: [" + parsingRequestLine
      ...
      if (socketWrapper != null) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Received [" + new String(byteBuffer.array(), byteBuffer.position(),
    ...
    private HeaderParseStatus parseHeader() throws IOException {
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_NAME) {
            ...
            if (chr == Constants.COLON) {
                ...
                headerData.headerValue = headers
                  .addValue(byteBuffer.array(), headerData.start, pos - headerData.start);
    ...
    private HeaderParseStatus skipLine() throws IOException {
        ...
        if (rejectIllegalHeader || log.isDebugEnabled()) {
            String message = sm.getString("iib.invalidheader", HeaderUtil
