...
public class AjpProcessor extends AbstractProcessor {
  ...
  public AjpProcessor(AbstractAjpProtocol<?> protocol, Adapter adapter) {
    ...
    System
      .arraycopy(getBodyMessage.getBuffer(), 0, getBodyMessageArray, 0, getBodyMessage.getLen());
  ...
  private void prepareRequest() {
    ...
    for (int i = 0; i < hCount; i++) {
      ...
      } else if (hId == Constants.SC_REQ_CONTENT_TYPE || (hId == -1 && tmpMB
    ...
    while ((attributeCode = requestHeaderMessage.getByte()) != Constants.SC_A_ARE_DONE) {
      ...
      switch (attributeCode) {
        ...
        case Constants.SC_A_SSL_KEY_SIZE:
          request
            .setAttribute(SSLSupport.KEY_SIZE_KEY, Integer.valueOf(requestHeaderMessage.getInt()));
          break;
