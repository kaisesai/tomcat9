...
public abstract class AbstractProtocol<S> implements ProtocolHandler, MBeanRegistration {
  ...
  protected abstract Processor createUpgradeProcessor(SocketWrapperBase<?> socket,
  ...
  public void init() throws Exception {
    ...
    if (this.domain != null) {
      ObjectName rgOname = new ObjectName(
      ...
      if (oname != null) {
        if (mserver == null) {
          ...
          try {
            ...
            getLog()
              .info(sm.getString("abstractProtocol.mbeanDeregistrationFailed", oname, mserver));
          }
        }
      }
  ...
  private void logPortOffset() {
    if (getPort() != getPortWithOffset()) {
      getLog().info(
        sm.getString("abstractProtocolHandler.portOffset", getName(), String.valueOf(getPort()),
  ...
  protected static class ConnectionHandler<S> implements AbstractEndpoint.Handler<S> {
    ...
    public SocketState process(SocketWrapperBase<S> wrapper, SocketEvent status) {
      if (getLog().isDebugEnabled()) {
        getLog()
          .debug(sm.getString("abstractConnectionHandler.process", wrapper.getSocket(), status));
      }
      if (wrapper == null) {
      ...
      // 不能接收的请求
      if (SocketEvent.TIMEOUT == status && (processor == null || !processor.isAsync() && !processor
        .isUpgrade() || processor.isAsync() && !processor.checkAsyncTimeoutGeneration())) {
      ...
      try {
        if (processor == null) {
          ...
          // 如果没有协商任何协议，则 openssl 会返回 null，JSSE 协议会返回 ""
          if (negotiatedProtocol != null && negotiatedProtocol.length() > 0) {
            ...
            if (upgradeProtocol != null) {
              ...
              if (getLog().isDebugEnabled()) {
                getLog()
                  .debug(sm.getString("abstractConnectionHandler.processorCreate", processor));
              }
