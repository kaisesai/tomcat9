...
class Http2AsyncParser extends Http2Parser {
  ...
  void readConnectionPreface(WebConnection webConnection, Stream stream) throws Http2Exception {
    ...
    PrefaceCompletionHandler handler = new PrefaceCompletionHandler(webConnection, stream,
                                                                    prefaceData, preface, header,
                                                                    framePayload);
    socketWrapper
      .read(BlockingMode.NON_BLOCK, socketWrapper.getReadTimeout(), TimeUnit.MILLISECONDS, null,
  ...
  protected boolean readFrame(boolean block, FrameType expected)
    ...
    CompletionState state = socketWrapper
      .read(block ? BlockingMode.BLOCK : BlockingMode.NON_BLOCK, socketWrapper.getReadTimeout(),
