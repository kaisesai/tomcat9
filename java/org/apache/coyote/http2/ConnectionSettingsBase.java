...
abstract class ConnectionSettingsBase<T extends Throwable> {
  ...
  final void set(Setting setting, long value) throws T {
    ...
    switch (setting) {
      ...
      case UNKNOWN:
        ...
        log.warn(
          sm.getString("connectionSettings.unknown", connectionId, setting, Long.toString(value)));
        return;
    }
  ...
  private void validateEnablePush(long enablePush) throws T {
    ...
    if (enablePush > 1) {
      String msg = sm
        .getString("connectionSettings.enablePushInvalid", connectionId, Long.toString(enablePush));
      throwException(msg, Http2Error.PROTOCOL_ERROR);
    }
  }
