      ...
      if (classLoader instanceof WebappProperties) {
        ...
        if (null != logConfig) {
          if (Boolean.getBoolean(DEBUG_PROPERTY)) {
            System.err.println(
              getClass().getName() + ".readConfiguration(): " + "Found logging.properties at "
                + logConfig);
          }
    ...
    if ((is == null) && (classLoader == ClassLoader.getSystemClassLoader())) {
      ...
      if (is == null) {
        File defaultFile = new File(
          new File(System.getProperty("java.home"), isJava9 ? "conf" : "lib"),
          "logging.properties");
        try {
    ...
    if (handlers != null) {
      ...
      while (tok.hasMoreTokens()) {
        ...
        try {
          ...
          Handler handler = (Handler) classLoader.loadClass(handlerClassName).getConstructor()
            .newInstance();
