...
public abstract class PojoEndpointBase extends Endpoint {
  ...
  protected final void doOnOpen(Session session, EndpointConfig config) {
    ...
    // session
    for (MessageHandler mh : methodMapping
      .getMessageHandlers(pojo, pathParameters, session, config)) {
      session.addMessageHandler(mh);
    }
    
    if (methodMapping.getOnOpen() != null) {
      try {
        methodMapping.getOnOpen()
          .invoke(pojo, methodMapping.getOnOpenArgs(pathParameters, session, config));
    ...
    if (methodMapping.getOnClose() != null) {
      try {
        methodMapping.getOnClose()
          .invoke(pojo, methodMapping.getOnCloseArgs(pathParameters, session, closeReason));
      } catch (Throwable t) {
  ...
  public final void onError(Session session, Throwable throwable) {
    ...
    if (methodMapping.getOnError() == null) {
      ...
      try {
        methodMapping.getOnError()
          .invoke(pojo, methodMapping.getOnErrorArgs(pathParameters, session, throwable));
      } catch (Throwable t) {
        ExceptionUtils.handleThrowable(t);
