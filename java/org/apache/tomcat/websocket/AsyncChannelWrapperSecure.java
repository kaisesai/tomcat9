...
public class AsyncChannelWrapperSecure implements AsyncChannelWrapper {
  ...
  private final ExecutorService executor = Executors
    .newFixedThreadPool(2, new SecureIOThreadFactory());
  ...
  @Override
  public <B, A extends B> void write(ByteBuffer[] srcs, int offset, int length, long timeout,
  ...
  private static final class LongToIntegerFuture implements Future<Integer> {
    ...
    public Integer get() throws InterruptedException, ExecutionException {
      ...
      if (result.longValue() > Integer.MAX_VALUE) {
        throw new ExecutionException(sm.getString("asyncChannelWrapperSecure.tooBig", result),
                                     null);
      }
    ...
    public void run() {
      ...
      try {
        ...
        if (writing.compareAndSet(true, false)) {
          ...
          future.fail(
            new IllegalStateException(sm.getString("asyncChannelWrapperSecure.wrongStateWrite")));
        }
      } catch (Exception e) {
        writing.set(false);
        future.fail(e);
    ...
    public void run() {
      ...
      try {
        ...
        if (reading.compareAndSet(true, false)) {
          ...
          future.fail(
            new IllegalStateException(sm.getString("asyncChannelWrapperSecure.wrongStateRead")));
        }
