...
public class WsWebSocketContainer implements WebSocketContainer, BackgroundProcess {
  ...
  public Session connectToServer(Object pojo, URI path) throws DeploymentException {
    ...
    if (!ClientEndpointConfig.Configurator.class.equals(configuratorClazz)) {
      try {
        ...
        throw new DeploymentException(sm.getString("wsWebSocketContainer.defaultConfiguratorFail"),
                                      e);
      }
    }
  ...
  public Session connectToServer(Class<?> annotatedEndpointClass, URI path)
    ...
    try {
      ...
      throw new DeploymentException(
        sm.getString("wsWebSocketContainer.endpointCreateFail", annotatedEndpointClass.getName()),
        e);
    }
  ...
  public Session connectToServer(Class<? extends Endpoint> clazz,
    ...
    try {
      ...
      throw new DeploymentException(
        sm.getString("wsWebSocketContainer.endpointCreateFail", clazz.getName()), e);
    }
  ...
  private Session connectToServerRecursive(Endpoint endpoint,
                                           ClientEndpointConfig clientEndpointConfiguration,
                                           URI path, Set<URI> redirectSet)
    throws DeploymentException {
    
    if (log.isDebugEnabled()) {
      log.debug(
        sm.getString("wsWebSocketContainer.connect.entry", endpoint.getClass().getName(), path));
    }
    ...
    try {
      ...
      throw new DeploymentException(
        sm.getString("wsWebSocketContainer.asynchronousSocketChannelFail"), ioe);
    }
    ...
    if (proxyConnect != null) {
      try {
        ...
        if (httpResponse.getStatus() != 200) {
          throw new DeploymentException(
            sm.getString("wsWebSocketContainer.proxyConnectFail", selectedProxy,
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        ...
        log.debug(
          sm.getString("wsWebSocketContainer.connect.write", Integer.valueOf(request.position()),
      ...
      if (httpResponse.status != 101) {
        if (isRedirectStatus(httpResponse.status)) {
          ...
          userProperties.put(Constants.AUTHORIZATION_HEADER_NAME, auth
            .getAuthorization(requestUri, wwwAuthenticateHeaders.get(0), userProperties));
      ...
      // Sub-protocol
      List<String> protocolHeaders = handshakeResponse.getHeaders()
        .get(Constants.WS_PROTOCOL_HEADER_NAME);
      ...
      List<String> extHeaders = handshakeResponse.getHeaders()
        .get(Constants.WS_EXTENSIONS_HEADER_NAME);
      if (extHeaders != null) {
      ...
      throw new DeploymentException(sm.getString("wsWebSocketContainer.httpRequestFailed", path),
                                    e);
    } finally {
      if (!success) {
        channel.close();
    ...
    WsSession wsSession = new WsSession(endpoint, wsRemoteEndpointClient, this, null, null, null,
                                        null, null, extensionsAgreed, subProtocol,
   ...
   */
  private HttpResponse processResponse(ByteBuffer response, AsyncChannelWrapper channel,
                                       long timeout)
    throws InterruptedException, ExecutionException, DeploymentException, EOFException,
           TimeoutException {
      ...
      if (sslContext == null) {
        ...
        if (sslTrustStoreValue != null) {
          String sslTrustStorePwdValue = (String) userProperties
            .get(Constants.SSL_TRUSTSTORE_PWD_PROPERTY);
