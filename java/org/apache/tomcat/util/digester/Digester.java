...
public class Digester extends DefaultHandler2 {
  ...
  public static void replaceSystemProperties() {
    ...
    if (propertySources != null) {
      ...
      for (String name : names) {
        ...
        if (value != null) {
          try {
            String newValue = IntrospectionUtils
              .replaceProperties(value, null, propertySources, null);
  ...
  public SAXException createSAXException(String message, Exception e) {
    ...
    if (locator != null) {
      String error = sm
        .getString("digester.errorLocation", Integer.valueOf(locator.getLineNumber()),
  ...
  private Attributes updateAttributes(Attributes list) {
    ...
    for (int i = 0; i < nAttributes; ++i) {
      ...
      try {
        newAttrs.setValue(i, IntrospectionUtils
          .replaceProperties(value, null, source, getClassLoader()).intern());
      } catch (Exception e) {
        log.warn(sm.getString("digester.failedToUpdateAttributes", newAttrs.getLocalName(i), value),
                 e);
      }
    }
