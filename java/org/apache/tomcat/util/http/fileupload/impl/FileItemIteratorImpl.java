...
public class FileItemIteratorImpl implements FileItemIterator {
  ...
  protected void init(FileUploadBase fileUploadBase,
                      @SuppressWarnings("unused") RequestContext pRequestContext)
    throws FileUploadException, IOException {
    ...
    try {
      ...
      IOUtils.closeQuietly(input); // avoid possible resource leak
      throw new InvalidContentTypeException(String.format(
        "The boundary specified in the %s header is too long", FileUploadBase.CONTENT_TYPE), iae);
    }
  ...
  public List<FileItem> getFileItems() throws FileUploadException, IOException {
    ...
    while (hasNext()) {
      ...
      final FileItem fi = fileUploadBase.getFileItemFactory()
        .createItem(fis.getFieldName(), fis.getContentType(), fis.isFormField(), fis.getName());
      items.add(fi);
    }
    return items;
  }
