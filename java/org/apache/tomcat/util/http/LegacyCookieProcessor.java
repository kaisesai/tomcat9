...
public final class LegacyCookieProcessor extends CookieProcessorBase {
  ...
  public String generateHeader(Cookie cookie, HttpServletRequest request) {
    ...
    if (version == 0) {
      ...
      if (needsQuotes(value, 0) || comment != null || needsQuotes(path, 0) || needsQuotes(domain,
                                                                                          0)) {
        version = 1;
      }
    }
      ...
      if (version == 0 || getAlwaysAddExpires()) {
        ...
        if (maxAge == 0) {
          ...
          COOKIE_DATE_FORMAT.get()
            .format(new Date(System.currentTimeMillis() + maxAge * 1000L), buf,
                    new FieldPosition(0));
        }
      }
    }
   ...
   * JVK
   */
  private final int getTokenEndPosition(byte bytes[], int off, int end, int version,
                                        boolean isName) {
    int pos = off;
    while (pos < end && (!isHttpSeparator((char) bytes[pos])
      || version == 0 && getAllowHttpSepsInV0() && bytes[pos] != '=' && !isV0Separator(
      (char) bytes[pos]) || !isName && bytes[pos] == '=' && getAllowEqualsInValue())) {
      pos++;
    }
