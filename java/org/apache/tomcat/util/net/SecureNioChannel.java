...
public class SecureNioChannel extends NioChannel {
  ...
  public SecureNioChannel(SocketBufferHandler bufHandler, NioSelectorPool pool,
  ...
  private int processSNI() throws IOException {
    ...
    while (extractor.getResult() == ExtractorResult.UNDERFLOW && netInBuffer.capacity() < endpoint
      .getSniParseLimit()) {
    ...
    netOutBuffer = ByteBufferUtils
      .expand(netOutBuffer, sslEngine.getSession().getPacketBufferSize());
  ...
  public void close() throws IOException {
    ...
    closed = (!netOutBuffer.hasRemaining() && (handshake.getHandshakeStatus()
  ...
  public int read(ByteBuffer dst) throws IOException {
    ...
    do {
      ...
      if (unwrap.getStatus() == Status.OK || unwrap.getStatus() == Status.BUFFER_UNDERFLOW) {
        ...
        if (read > 0) {
          ...
          } else if (getAppReadBufHandler() != null && dst == getAppReadBufHandler()
            .getByteBuffer()) {
    ...
    while (netInBuffer.position()
      != 0); //continue to unwrapping as long as the input buffer has stuff
    return read;
  }
