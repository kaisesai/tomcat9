...
public abstract class SSLUtilBase implements SSLUtil {
  ...
  protected SSLUtilBase(SSLHostConfigCertificate certificate, boolean warnTls13) {
    ...
    if (!implementedProtocols.contains(Constants.SSL_PROTO_TLSv1_3) && !sslHostConfig
      .isExplicitlyRequestedProtocol(Constants.SSL_PROTO_TLSv1_3)) {
    ...
    if (!implementedProtocols.contains(Constants.SSL_PROTO_SSLv2Hello) && !sslHostConfig
      .isExplicitlyRequestedProtocol(Constants.SSL_PROTO_SSLv2Hello)) {
      configuredProtocols.remove(Constants.SSL_PROTO_SSLv2Hello);
    }
    
    List<String> enabledProtocols = getEnabled("protocols", getLog(), warnTls13,
                                               configuredProtocols, implementedProtocols);
    ...
    List<String> enabledCiphers = getEnabled("ciphers", getLog(), false, configuredCiphers,
                                             implementedCiphers);
  ...
  static KeyStore getStore(String type, String provider, String path, String pass)
    ...
    try {
      ...
      if ("DKS".equalsIgnoreCase(type)) {
        ...
        char[] storePass = null;
        if (pass != null && (!"".equals(pass) || "JKS".equalsIgnoreCase(type) || "PKCS12"
  ...
  public KeyManager[] getKeyManagers() throws Exception {
    ...
    if (ks == null) {
      ...
      PEMFile privateKeyFile = new PEMFile(
        ...
        certificate.getCertificateFile(), keyPass);
      ...
      if (k != null && !"DKS".equalsIgnoreCase(certificate.getCertificateKeystoreType()) && "PKCS#8"
        .equalsIgnoreCase(k.getFormat())) {
        // Switch to in-memory key store
