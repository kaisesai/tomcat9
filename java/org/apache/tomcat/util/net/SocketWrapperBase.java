...
public abstract class SocketWrapperBase<E> {
  ...
  protected void writeNonBlocking(byte[] buf, int off, int len) throws IOException {
    if (len > 0 && nonBlockingWriteBuffer.isEmpty() && socketBufferHandler
  ...
  protected void writeNonBlocking(ByteBuffer from) throws IOException {
    ...
    if (from.hasRemaining() && nonBlockingWriteBuffer.isEmpty() && socketBufferHandler
      .isWriteBufferWritable()) {
   ...
   * @return the completion state (done, done inline, or still pending)
   */
  public final <A> CompletionState read(BlockingMode block, long timeout, TimeUnit unit,
   ...
   * @return the completion state (done, done inline, or still pending)
   */
  public final <A> CompletionState write(BlockingMode block, long timeout, TimeUnit unit,
                                         A attachment, CompletionCheck check,
                                         CompletionHandler<Long, ? super A> handler,
                                         ByteBuffer... srcs) {
    if (srcs == null) {
   ...
   * @return the completion state (done, done inline, or still pending)
   */
  protected final <A> CompletionState vectoredOperation(boolean read, ByteBuffer[] buffers,
    ...
    } else if (!hasPerOperationTimeout() && (unit.toMillis(timeout) != (read ? getReadTimeout() :
                                                                        getWriteTimeout()))) {
      if (read) {
    ...
    if (block == BlockingMode.BLOCK || block == BlockingMode.SEMI_BLOCK) {
      ...
      if (read ? !readPending.tryAcquire() : !writePending.tryAcquire()) {
        if (block == BlockingMode.NON_BLOCK) {
          ...
          handler
            .failed(read ? new ReadPendingException() : new WritePendingException(), attachment);
          return CompletionState.ERROR;
        }
      }
    }
    ...
    OperationState<A> state = newOperationState(read, buffers, offset, length, block, timeout, unit,
                                                ...
                                                read ? readPending : writePending, completion);
    if (read) {
      readOperation = state;
    } else {
  ...
  protected abstract <A> OperationState<A> newOperationState(boolean read, ByteBuffer[] buffers,
                                                             int offset, int length,
                                                             BlockingMode block, long timeout,
  ...
  protected class VectoredIOCompletionHandler<A>
    ...
    public void completed(Long nBytes, OperationState<A> state) {
      if (nBytes.longValue() < 0) {
        ...
        if (state.check != null) {
          CompletionHandlerCall call = state.check
            .callHandler(currentState, state.buffers, state.offset, state.length);
