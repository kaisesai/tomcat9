  ...
  public static class Nio2SocketWrapper extends SocketWrapperBase<Nio2Channel> {
    ...
    private CompletionHandler<Integer, SendfileData> sendfileHandler
      ...
      public void completed(Integer nWrite, SendfileData attachment) {
          ...
          if (attachment.length <= 0) {
            ...
            if (isInline()) {
              ...
              switch (attachment.keepAliveState) {
                case NONE: {
                  getEndpoint()
                    .processSocket(Nio2SocketWrapper.this, SocketEvent.DISCONNECT, false);
                  break;
                }
                case PIPELINED: {
                  if (!getEndpoint()
                    .processSocket(Nio2SocketWrapper.this, SocketEvent.OPEN_READ, true)) {
                    close();
                  }
                  break;
                }
        ...
        getSocket()
          .write(buffer, toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS, attachment, this);
      }
      ...
      this.readCompletionHandler = new CompletionHandler<Integer, ByteBuffer>() {
        ...
        public void failed(Throwable exc, ByteBuffer attachment) {
          ...
          if (exc instanceof AsynchronousCloseException) {
            ...
            .processSocket(Nio2SocketWrapper.this, SocketEvent.ERROR, true)) {
            close();
          }
        }
      };
        ...
        public void completed(Integer nBytes, ByteBuffer attachment) {
          ...
          synchronized (writeCompletionHandler) {
            if (nBytes.intValue() < 0) {
              ...
              getSocket()
                .write(array, 0, array.length, toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS,
                       array, gatheringWriteCompletionHandler);
            } else if (attachment.hasRemaining()) {
              // Regular write
              getSocket()
                .write(attachment, toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS, attachment,
        ...
        public void completed(Long nBytes, ByteBuffer[] attachment) {
          ...
          synchronized (writeCompletionHandler) {
            if (nBytes.longValue() < 0) {
              failed(new EOFException(sm.getString("iob.failedwrite")), attachment);
            } else if (!nonBlockingWriteBuffer.isEmpty() || buffersArrayHasRemaining(attachment, 0,
                                                                                     attachment.length)) {
              // Continue writing data using a gathering write
              ByteBuffer[] array = nonBlockingWriteBuffer.toArray(attachment);
              getSocket()
    ...
    protected <A> OperationState<A> newOperationState(boolean read, ByteBuffer[] buffers,
      ...
      return new Nio2OperationState<>(read, buffers, offset, length, block, timeout, unit,
    ...
    private int fillReadBuffer(boolean block, ByteBuffer to) throws IOException {
      ...
      if (block) {
        ...
        getSocket()
          .read(to, toTimeout(getReadTimeout()), TimeUnit.MILLISECONDS, to, readCompletionHandler);
        ...
        if (writeNotify || hasPermit || writePending.tryAcquire()) {
          ...
          if (!nonBlockingWriteBuffer.isEmpty()) {
            ByteBuffer[] array = nonBlockingWriteBuffer
              .toArray(socketBufferHandler.getWriteBuffer());
            Nio2Endpoint.startInline();
            getSocket()
              .write(array, 0, array.length, toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS,
                     array, gatheringWriteCompletionHandler);
    ...
    public SendfileState processSendfile(SendfileDataBase sendfileData) {
      ...
      if (nRead >= 0) {
        ...
        getSocket().getBufHandler().configureWriteBufferForRead();
        Nio2Endpoint.startInline();
        getSocket().write(buffer, toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS, data,
                          sendfileHandler);
        Nio2Endpoint.endInline();
        if (data.doneInline) {
          if (data.error) {
                ...
                getSocket().write(array, 0, array.length, timeout, unit, array,
                                  new CompletionHandler<Long, ByteBuffer[]>() {
                                    ...
                                    public void completed(Long nBytes, ByteBuffer[] buffers) {
                                      ...
                                      } else if (buffersArrayHasRemaining(buffers, 0,
                                                                          buffers.length)) {
