...
public class Diagnostics {
  ...
  public static void setThreadContentionMonitoringEnabled(boolean enable) {
    ...
    if (enable != checkValue) {
      log.error(
        "Could not set threadContentionMonitoringEnabled to " + enable + ", got " + checkValue
          + " instead");
    }
  }
  ...
  public static void setLoggerLevel(String loggerName, String levelName) {
    ...
    if (!checkValue.equals(levelName)) {
      log.error(
        "Could not set logger level for logger '" + loggerName + "' to '" + levelName + "', got '"
          + checkValue + "' instead");
    }
  }
  ...
  public static void setVerboseGarbageCollection(boolean verbose) {
    ...
    if (verbose != checkValue) {
      log.error(
        "Could not set verbose garbage collection logging to " + verbose + ", got " + checkValue
          + " instead");
    }
  }
  ...
  private static String getThreadDump(ThreadInfo ti) {
    ...
    for (int i = 0; i < stes.length; i++) {
      ...
      if (monitorDepths[i] != null) {
        ...
        sb.append(
          INDENT2 + "- locked (a " + mi.toString() + ")" + " index " + mi.getLockedStackDepth()
  ...
  public static String getVMInfo(StringManager requestedSm) {
    ...
    sb.append(":" + CRLF);
    sb.append(
      INDENT1 + "isCurrentThreadCpuTimeSupported: " + threadMXBean.isCurrentThreadCpuTimeSupported()
        + CRLF);
    sb.append(
    ...
    sb.append(
      INDENT1 + "isObjectMonitorUsageSupported: " + threadMXBean.isObjectMonitorUsageSupported()
        + CRLF);
    sb.append(
      INDENT1 + "isSynchronizerUsageSupported: " + threadMXBean.isSynchronizerUsageSupported()
        + CRLF);
    sb.append(INDENT1 + "isThreadContentionMonitoringSupported: " + threadMXBean
      .isThreadContentionMonitoringSupported() + CRLF);
    sb.append(INDENT1 + "isThreadContentionMonitoringEnabled: " + threadMXBean
      .isThreadContentionMonitoringEnabled() + CRLF);
    sb.append(CRLF);
    ...
    sb.append(INDENT1 + "isCompilationTimeMonitoringSupported: " + compilationMXBean
      .isCompilationTimeMonitoringSupported() + CRLF);
    sb.append(CRLF);
    ...
    for (MemoryPoolMXBean mbean : memoryPoolMXBeans) {
      ...
      try {
        sb.append(INDENT1 + "isCollectionUsageThresholdExceeded: " + mbean
          .isCollectionUsageThresholdExceeded() + CRLF);
      } catch (UnsupportedOperationException ex) {
        // IGNORE
      }
      try {
      ...
      try {
        sb.append(
          INDENT1 + "getCollectionUsageThresholdCount: " + mbean.getCollectionUsageThresholdCount()
            + CRLF);
      } catch (UnsupportedOperationException ex) {
        // IGNORE
      }
