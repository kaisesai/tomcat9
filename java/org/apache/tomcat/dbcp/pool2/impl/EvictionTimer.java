...
class EvictionTimer {
   ...
   */
  static synchronized void schedule(final BaseGenericObjectPool<?>.Evictor task, final long delay,
                                    final long period) {
    if (null == executor) {
    ...
    final ScheduledFuture<?> scheduledFuture = executor
      .scheduleWithFixedDelay(runner, delay, period, TimeUnit.MILLISECONDS);
  ...
  private static class EvictorThreadFactory implements ThreadFactory {
    ...
    public Thread newThread(final Runnable runnable) {
      ...
      thread
        .setDaemon(true); // POOL-363 - Required for applications using Runtime.addShutdownHook().
