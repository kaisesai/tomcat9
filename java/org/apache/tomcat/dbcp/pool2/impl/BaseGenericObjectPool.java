  ...
  public final void setEvictionPolicyClassName(final String evictionPolicyClassName,
    ...
    try {
      ...
      throw new IllegalArgumentException(
        "Class " + evictionPolicyClassName + " from class loaders [" + classLoader + ", "
          + epClassLoader + "] do not implement " + EVICTION_POLICY_TYPE_NAME);
      ...
      if (evictor == null) { // Starting evictor for the first time or after a cancel
        ...
        if (delay > 0) { // Restart
          synchronized (EvictionTimer.class) { // Ensure no cancel can happen between cancel / schedule calls
            EvictionTimer
              .cancel(evictor, evictorShutdownTimeoutMillis, TimeUnit.MILLISECONDS, true);
            evictor = null;
  ...
  private ObjectName jmxRegister(final BaseObjectPoolConfig<T> config, final String jmxNameBase,
    ...
    while (!registered) {
      try {
        ...
        if (BaseObjectPoolConfig.DEFAULT_JMX_NAME_PREFIX.equals(jmxNamePrefix) && jmxNameBase
          .equals(base)) {
