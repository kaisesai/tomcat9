...
class PoolImplUtils {
  ...
  static Class<?> getFactoryType(final Class<? extends PooledObjectFactory> factoryClass) {
    ...
    if (genericType instanceof Integer) {
      ...
      if (pi != null) {
        final Type[] bounds = ((TypeVariable) pi.getActualTypeArguments()[((Integer) genericType)
          .intValue()]).getBounds();
  ...
  private static <T> Object getGenericType(final Class<T> type, final Class<? extends T> clazz) {
    ...
    @SuppressWarnings("unchecked") final Class<? extends T> superClass = (Class<? extends T>) clazz
      .getSuperclass();
   ...
   */
  private static <T> ParameterizedType getParameterizedType(final Class<T> type,
