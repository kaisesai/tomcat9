...
public class ELProcessor {
  ...
  public void setVariable(String variable, String expression) {
    if (expression == null) {
      ...
      ValueExpression ve = factory
        .createValueExpression(context, bracket(expression), Object.class);
  ...
  public void defineFunction(String prefix, String function, Method method)
    ...
    if (!Modifier.isStatic(modifiers) || !jreCompat.canAccess(null, method)) {
      throw new NoSuchMethodException(Util
                                        .message(context, "elProcessor.defineFunctionInvalidMethod",
                                                 method.getName(),
        ...
        if (paramString.length() == 0) {
          ...
          for (int i = 0; i < parameterTypeNames.length; i++) {
            ...
            if (parameterTypeName.endsWith("...")) {
              varArgs = true;
              dimension = 1;
              parameterTypeName = parameterTypeName.substring(0, parameterTypeName.length() - 3)
                .trim();
            }
